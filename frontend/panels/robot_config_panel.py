...
class RobotConfigPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.preset_combo.setStyleSheet("background: #444; color: #ddd; padding: 4px;")
        self.preset_combo.currentTextChanged.connect(self._load_preset)
        layout.addWidget(self.preset_combo)

        # Parameter group
        group = QGroupBox("Dimensions")
        group.setStyleSheet("QGroupBox { color: #aaa; font-size: 11px; border: 1px solid #555; margin-top: 8px; } QGroupBox::title { subcontrol-origin: margin; left: 8px; padding: 0 4px; }")
        group_layout = QVBoxLayout(group)

        # Spinbox style
        sb_style = """
            QDoubleSpinBox { background: #333; color: #eee; padding: 4px; border: 1px solid #555; }
        ...
        btn_reset.setStyleSheet("background: #555; color: #ddd; padding: 6px;")
    ...
    def _load_preset(self, name):
        ...
        if name in self.PRESETS:
            self.config = self.PRESETS[name].__class__(**self.PRESETS[name].__dict__)  # copy
            self._apply_config_to_spinboxes()
            self.config_changed.emit(self.config)
    ...
    def set_config(self, config: ArmConfig):
        """Update entire config (from external source)."""
        self.config = config
        self._apply_config_to_spinboxes()
